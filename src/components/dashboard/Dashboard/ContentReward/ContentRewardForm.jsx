...
const CampaignForm = ({
	...
	const {data: campaignTypesData, isLoading: isTypesLoading} =
		useGetCampaignTypes();
	...
	const isInitialLoading =
		isTypesLoading || isCategoriesLoading || isCurrenciesLoading;
	...
	const selectedCurrency = currencies.find(
		(c) =>
			c.code.toUpperCase() ===
			(formData?.currency?.toUpperCase() || "USD")
	);
	...
	useEffect(() => {
		const validate = () => {
			...
			const isBasicAndNumericValid = requiredFields.every(
				(field) => field
			);
			...
			const isGoogleDriveLink = (url) => {
				...
				return /^(https?:\/\/)?(www\.)?(drive|docs)\.google\.com\/.+$/.test(
					url
				);
			};

			const isContentLinkValid = isGoogleDriveLink(
				formData.availableContent
			);
			const isTermsAccepted = formData.termsAccepted;
			const isFlatFeeValid =
				!formData.flatFeeBonus ||
				Number(formData.flatFeeBonus) <=
					Number(formData.campaignBudget) * 0.1;
			...
			<form onSubmit={handleSubmit} className="space-y-8">
				...
				<div className="space-y-2">
					...
					<div className="relative">
						<input
							...
							onChange={(e) =>
								handleInputChange("rewardRate", e.target.value)
							}
							placeholder="0.00"
							className="w-full px-3 py-3 text-foreground-strong bg-secondary border border-border rounded-lg focus:ring-2 focus:ring-foreground-muted focus:border-transparent transition-all outline-none placeholder:text-foreground-muted disabled:opacity-50 disabled:cursor-not-allowed"
						/>
						...
						<label className="text-sm font-medium text-foreground-strong mb-2 flex items-center gap-2">
							...
							<Popover>
								...
								<PopoverContent className="w-72 text-sm">
									If enabled, users must reach this minimum
									amount before becoming eligible for payout.
								</PopoverContent>
							</Popover>
						</label>
						<div className="relative">
					...
					<div className="space-y-2">
						<label className="text-sm font-medium text-foreground-strong mb-2 flex items-center gap-2">
							...
							<Popover>
								...
								<PopoverContent className="w-72 text-sm">
									If enabled, users cannot earn more than this
									specified amount for each submission.
								</PopoverContent>
							</Popover>
						</label>
						<div className="relative">
				...
				<div className="space-y-2">
					<label className="text-sm font-medium text-foreground-strong mb-4 flex items-center gap-2">
						...
						<Popover>
							...
							<PopoverContent className="w-72 text-sm">
								This is a fixed bonus amount per submission,
								which will be paid once the submission is
								approved.
							</PopoverContent>
						</Popover>
					</label>
						...
						{formData.flatFeeBonus &&
							Number(formData.flatFeeBonus) >
								...
								<p className="text-xs text-red-500 mt-1">
									Flat fee bonus cannot exceed 10% of the
									total budget ($
									{(
										Number(formData.campaignBudget) * 0.1
									).toFixed(2)}
				...
				<div className="space-y-2">
					...
					<p className="text-xs text-foreground-muted">
						We suggest adding your guides, raw footage, and other
						materials for creators to edit with here via a Google
						Drive link
					</p>
					<div className="relative">
						<input
				...
				<div className="space-y-2">
					...
					<p className="text-xs text-foreground-muted">
						We recommend including content guidelines for users to
						follow.
					</p>
					<textarea
					...
					<label className="text-sm font-medium text-foreground-strong mb-4 flex items-center gap-2">
						...
						<Popover>
							...
							<PopoverContent className="w-72 text-sm">
								...
								The campaign will only be active during this
								period.
							</PopoverContent>
						</Popover>
					</label>
				...
				{!isEditMode && (
					<div className="flex items-start space-x-3 p-4 bg-secondary rounded-xl border border-border transition-colors">
						...
						<div className="text-sm">
							<label
								htmlFor="terms"
								className="font-medium text-foreground-strong cursor-pointer"
							>
								I agree to the{" "}
								<a
							...
							<p className="text-foreground-muted mt-1 text-sm">
								You must accept the terms before creating a new
								content reward campaign.
							</p>
						</div>
					</div>
				)}
				...
				<div className="flex flex-col sm:flex-row gap-4 pt-4">
					<Button
						...
						className={`transition font-medium px-6 py-4 rounded-lg text-white ${
