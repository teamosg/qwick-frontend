...
const ToggleSwitch = ({ enabled, onToggle, disabled = false }) => (
    <button
        disabled={disabled}
        onClick={onToggle}
        className={`relative cursor-pointer disabled:cursor-not-allowed inline-flex h-6 w-11 items-center rounded-full transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ${enabled ? "bg-primary shadow-lg" : "bg-muted hover:bg-muted"
