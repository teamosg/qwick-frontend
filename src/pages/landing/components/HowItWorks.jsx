...
const lessons = [
  ...
  { 
    ...
    videoUrl: 'https://cdn.coverr.co/videos/coverr-typing-on-a-laptop-4482/1080p.mp4'
  },
  { 
    id: 3, 
...
const HowItWorks = () => {
  ...
  return (
    <section className="py-24 bg-white border-t border-slate-100" id="video-guides">
      <div className="container mx-auto px-6 text-center mb-16">
        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-4 font-inter">Video Guide</span>
        <h2 className="text-3xl md:text-5xl font-bold mb-6 font-inter tracking-tighter ">Learn Your Way Around</h2>
        <p className="text-slate-500 max-w-xl mx-auto font-inter">
Follow our quick, step-by-step video guides and get comfortable using the platform in minutes, no guesswork needed.        </p>
      </div>
      ...
      <div className="container mx-auto px-6">
        <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
            ...
            {isPlaying ? (
              ...
              <>
                ...
                <div className="absolute inset-0 flex items-center justify-center z-10">
                  ...
                  >
                    <Play size={32} className="text-white fill-white ml-2 group-hover/play:scale-110 transition-transform" />
                  </div>
                </div>
          ...
          <div className="lg:w-1/3 space-y-4">
            <div className="text-xs font-bold text-slate-400  tracking-widest mb-4 flex items-center justify-between">
              ...
              {lessons.map((lesson) => (
                ...
                >
                  <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                    activeLessonId === lesson.id ? 'bg-white/10' : 'bg-slate-100'
                  }`}>
                    {activeLessonId === lesson.id ? <Play size={20} className="fill-white" /> : <Play size={20} className="text-slate-400" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2 mb-0.5">
                      <span className="font-bold truncate text-sm">{lesson.title}</span>
                      <span className={`text-[10px] font-bold ${activeLessonId === lesson.id ? 'text-white/60' : 'text-slate-400'}`}>
                        {lesson.duration}
                      </span>
                    </div>
                    <p className={`text-[10px] truncate ${activeLessonId === lesson.id ? 'text-white/40' : 'text-slate-400'}`}>
                      {lesson.description}
                    </p>
                  </div>
                </button>
